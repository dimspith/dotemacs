...
** Xterm-color
A better replacement for ansi-color.el which translates ansi escape sequences to colors.
   #+BEGIN_SRC emacs-lisp
...
** Zetteldeft
A zettelkasten system built on top of the =deft= package. We load it after deft and set it's classic keybindings, which start with =C-c d=
#+BEGIN_SRC emacs-lisp
...
** Marginalia
Adds annotations at the margin of the minibuffer for completion candidates of selectrum such as files or command descriptions.
#+begin_src emacs-lisp
...
("clangd" "--background-index" "--suggest-missing-includes" "--all-scopes-completion" "--completion-style=detailed")))
...
** Simple-Modeline
A simple modeline for emacs that is slightly more beautiful than the default.
#+BEGIN_SRC emacs-lisp
...
** Themes
Doom-themes is a collection of themes made by the community.
#+BEGIN_SRC emacs-lisp
